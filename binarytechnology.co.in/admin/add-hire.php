<?php 
	...
	$name = '' ;
//	$for  = '';
	if(isset($attributes['addhire']) && $attributes['addhire'] !=''){
		$error = $adminFunction->addHire();
...
//		$for  = $attributes['cmbCategoryFor'];
	...
	if($attributes['act']=='edithire'){
		$catdetails = $adminFunction->getHireDetails($attributes['categoryId']);
...
//		$for = $catdetails[0]['categoryFor'];
	...
	if(isset($attributes['edithire']) && $attributes['edithire'] !=''){
		$error = $adminFunction->addHire();
...
<table width="95%" border="0" cellpadding="0" cellspacing="1" id="inner1">
	<tr>
        <th><?php if($attributes['act']!='edithire'){?>
				Add Link 
			<?php }else {?>
				Edit Link 
			<?php } ?> <br>
    ...
    <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
	   ...
	   <tr>
            ...
            <!--<textarea name="txtNews" cols="70" rows="25" title="News::Please enter news."><?php echo $description; ?></textarea>-->
            <?php
     ...
     <tr>
      ...
      <tr>
        <td align="right">&nbsp;</td>
        <td align="left">
		<?php if($attributes['act']!='edithire'){?>
			<input type="image" name="submit" alt="Add" src="images/add.gif" />
			<input type="hidden" name="addhire" value="addhire" />
		<?php } else {?>
			<input type="image" name="submit" alt="Edit" src="images/update.gif" />
			<input type="hidden" name="edithire" value="edithire" />
		...
		<input type="hidden" name="lstart" value="<?php echo $attributes['lstart'];?>"/>
			<a href="index.php?act=managehire&limitstart=<?php 
