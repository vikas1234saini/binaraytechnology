<?php 
	...
	$category  = '';
//	print_r($attributes);
	$categories = $adminFunction->getServicesDetails("","","categoryParent='0'");

	if(isset($attributes['addsubservices']) && $attributes['addsubservices'] !=''){
		echo "dcsd";
		$error = $adminFunction->addSubServices();
	...
	if($attributes['act']=='editsubservices'){
		$catdetails = $adminFunction->getServicesDetails($attributes['subCategoryId']);
	...
	if(isset($attributes['editsubservices']) && $attributes['editsubservices'] !=''){
		$error = $adminFunction->addSubServices();
...
<table width="95%" border="0" cellpadding="0" cellspacing="1" id="inner1">
	<tr>
        <th>
			<?php if($attributes['act']!='addsubservices'){?>
				Add Sub Category 
			<?php }else {?>
				Edit Sub Category 
			<?php } ?>
    ...
    <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
     ...
     <tr>
      ...
      <tr>
        <td align="right">&nbsp;</td>
        <td align="left">
		<?php if($attributes['act']!='editsubservices'){?>
			<input type="image" name="submit" alt="Add" src="images/add.gif" />
			<input type="hidden" name="addsubservices" value="addsubservices" />
		<?php } else {?>
			<input type="image" name="submit" alt="Edit" src="images/update.gif" />
			<input type="hidden" name="editsubservices" value="editsubservices" />
		...
		<a href="index.php?act=managesubservices&limitstart=<?php echo $attributes['lstart'];?>"><img src="images/back.gif" alt="Back" border="0" /></a>
		</td>
      </tr>
    </table></td>
  </tr>
